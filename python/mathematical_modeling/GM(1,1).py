import numpy as np
import sympy as sp
filepath = "D:\\MATLAB\\2021b\\Workplace\\人口.xls"
...
# .T表示转置，vstack函数理解为“缝合怪”（上下），矩阵里(2,)与(2,1)等价
B = np.vstack([-z, np.ones(n-1)]).T
u = np.linalg.pinv(B) @ x0[1:]  # 最小二乘法拟合参数
sp.var('t')
sp.var('x', cls=sp.Function)  # 定义符号变量和函数
eq = x(t).diff(t)+u[0]*x(t)-u[1]  # 定义符号微分方程，diff是求导函数
xt0 = sp.dsolve(eq, ics={x(0): x0[0]})  # 求解符号微分方程，ics是为了t = 0时，等式成立，不然多了一个常数
